...
function loadSeason() {
  try {
    season = seasonLib.normalizeSeason(JSON.parse(fs.readFileSync(SEASON_FILE, 'utf8')));
    console.log(`已恢复赛季战绩：${Object.keys(season.players).length} 名玩家`);
  } catch { /* 首次启动或数据损坏，从空赛季开始 */ }
}
...
function loadRooms() {
  try {
    ...
    for (const room of raw.rooms) {
      ...
      // 旧版本存档里的房间没有 seasonRecorded 标记：它们早在结束那一刻就该被计入，
      // 这里直接补标记，避免重连结算房时 recordRoom 把同一局重复累计进赛季战绩。
      if (room.phase === 'ended') room.seasonRecorded = true;
...
function broadcast(room) {
  ...
  // recordRoom 按房间幂等（seasonRecorded），结算广播重入/重连都不会重复计入。
